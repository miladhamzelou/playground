...
class TelethonTestCase(unittest.TestCase):
    ...
    def test_download_v(self):
        loop = asyncio.get_event_loop()
        loop.run_until_complete(self.download_dialogs("v", "+8613732215927"))

    def test_download_w(self):
        loop = asyncio.get_event_loop()
        loop.run_until_complete(self.download_dialogs("w", "+8613868136443"))
                    ...
                    async for message in client.iter_messages(dialog.entity, limit=10, min_id=self.config[key], reverse=True):
                        ...
                        for attr in media.document.attributes:
                            ...
                            while [ True ]:
                                ...
                                asyncio.create_task(client.download_media(message, file_name))
                                break
                        self.config[key]= message.id
        finally:
    ...
    async def download_dialogs(self, account, phone):
        ...
        # proxy = (socks.HTTP, 'phobos.public.opennetwork.cc', 1090, False,
        #  '353916769', 'dN2TTGIm')
